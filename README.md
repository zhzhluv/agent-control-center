...
- **프로젝트별 세션 뷰** - Claude/Codex가 작업 중인 프로젝트 시각화
- **Source Badge** - Claude(파랑)/Codex(주황) 세션 구분 표시
...
3. **로컬 Claude/Codex 세션**: 터미널/VSCode/Cursor에서 직접 실행한 Claude Code 또는 Codex 세션
...
│       ├── codex-monitor.ts   # Codex 세션 파일 감시
│       └── auth.ts            # 인증
├── client/          # React 프론트엔드
│   └── src/
│       ├── App.tsx                    # 메인 앱
...
- **Credential 비접근**: `~/.claude/auth.json`, `~/.codex/auth.json` 등 credential 파일은 절대 읽지 않음. 세션 로그 파일(`.jsonl`)만 파싱
